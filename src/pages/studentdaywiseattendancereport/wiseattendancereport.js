...
const Studentdaywiseattendancereport = () => {
  ...
  const [attendanceSplit, setAttendanceSplit] = useState([]);
  ...
  const exportToPDF = () => {
    ...
    attendanceData.forEach((row) => {
      ...
      const formattedDate = row.date_of_join
        ? row.date_of_join.split(" ")[0].split("-").reverse().join("-")
        : "";
  ...
  try {
    const response = await axios.post(
      baseUrl + "/studentdaywiseattendancereport/",
      {
        action: "FILTER",
    ...
    // Directly map morning/afternoon fields to am/pm
    const mapped = data.map((row) => ({
      ...row,
      am_attendance: row.morning_present,
      pm_attendance: row.afternoon_present,
    }));
  ...
  const columns = [
   ...
   {
  name: "AM Attendance",
  selector: (row) => row.am_attendance,
  sortable: true,
  center: true,
  width: "230px",
  cell: (row) =>
...
{
  name: "PM Attendance",
  selector: (row) => row.pm_attendance,
  sortable: true,
  center: true,
  width: "230px",
  cell: (row) =>
  ...
  const filteredRecords = (attendanceData || []).filter((student) => {
    return (
      ["admission_number", "student_name", "roll_no"].some(
        (column) =>
          student[column] &&
  ...
  const exportToExcel = () => {
        ...
        ? Object.keys(attendanceData[0]).map((key) => {
            const maxLength = Math.max(
              ...
              ...attendanceData.map((student) =>
                student[key] ? student[key].toString().length : 0
              )
            );
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "DayWiseReport");
    XLSX.writeFile(
      workbook,
      `student_day_wise_attendance_report_${new Date().toISOString()}.xlsx`
    );
  };
                  ...
                  <Row>
                    {/* <Col xs={12} md={6} lg={3} xxl={3}>
                      <div className="commonInput">
                        <Form.Group>
                          <Form.Label>
                            Academic Year<span className="requiredStar">*</span>
                          </Form.Label>
                          <select
                      ...
                      <div className="commonInput">
                        <Form.Group>
                          ...
                          >
                            ...
                            {(classes || [])
                              .filter(
                                (classItem) => classItem.is_active === "Active"
                              ) // Filter to include only active classes
                              .map((classItem) => (
                                <option
