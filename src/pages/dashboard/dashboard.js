...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (trendClass && trendSection && selectedMonth) {
      fetchAttendanceTrendData(trendClass, trendSection, selectedMonth);
    }
  ...
  const filteredClassFeeData = classFeeData.filter((item) =>
    item.student_name.toLowerCase().includes(classFeeSearch.toLowerCase()) ||
    String(item.roll_no).includes(classFeeSearch)
  );
  ...
  const fetchClassTeacherSections = async () => {
    try {
      const response = await axios.get(`${baseUrl}/get_sections_by_class_teacher`, {
        params: {
          class_teacher_id: userObj.user_id,
          school_id: userObj.school_id
        }
      });

      const data = response.data || [];
      setTeacherClassSections(data);

      if (data.length > 0) {
        const { class_id, section_id } = data[0];

        setClassFeeClass(class_id);
        setClassFeeSection(section_id);

        setAttendanceClass(class_id);
        setAttendanceSection(section_id);

        setMonthlyClass(class_id);
        setMonthlySection(section_id);

        setTrendClass(class_id);
        setTrendSection(section_id);
      }
    } catch (error) {
      console.error("Error fetching class teacher sections:", error);
    }
  };
  ...
  useEffect(() => {
    if (userObj?.user_id && userObj?.school_id) {
      fetchClassTeacherSections();
    }
  }, [userObj]);
          ...
          <Row>
            <Col md={6} className="mb-4">
              <Card>
                    ...
                    onClick={() =>
                      handleMaximize(
                        ...
                        <TimetableTable
                          periods={periods}
                          weekdays={weekdays}
                          timetableGrid={processTimetableData()}
                ...
                <Card.Body style={{ fontSize: '12px', overflowX: 'auto', padding: "0px" }}>
                  {weekdays.length > 0 && periods.length > 0 && timetable.length > 0 ? (
                    <TimetableTable
                      periods={periods}
                      weekdays={weekdays}
                      timetableGrid={processTimetableData()}
                    ...
                    <p className="text-muted text-center mt-3">No timetable available</p>
                  )}
                </Card.Body>
              </Card>
            </Col>
            <Col md={6} className="mb-4" style={{ maxHeight: '390px', overflowY: 'auto' }}>
              <Card>
                <Card.Header
